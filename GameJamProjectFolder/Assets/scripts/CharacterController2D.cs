...
public class CharacterController2D : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        var colliderWidth = _boxCollider.size.x * Math.Abs(transform.localScale.x) - (2 * SkinWidth);
        _horizontalDistanceBetweenRays = colliderWidth / (TotalVerticalRays - 1);

        var colliderHeight = _boxCollider.size.y * Math.Abs(transform.localScale.y) - (2 * SkinWidth);
    ...
    private void Move(Vector2 deltaMovement)
    {
        ...
        if (HandleCollisions)
        {
            ...
            if (Math.Abs(deltaMovement.x) > 0.001f)
